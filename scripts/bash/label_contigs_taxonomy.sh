...
awk -F'\t' '{gsub(/^ +/, "", $6); print $5 "\t" $6}' \
    data/genomes/bacterial/taxonomy/hq_chromosomes.report \
    > data/genomes/bacterial/taxonomy/taxid_to_name.tsv

# build contig -> taxID lookup from kraken2 output
awk -F'\t' '{print $2 "\t" $3}' \
    data/genomes/bacterial/taxonomy/hq_chromosomes.out \
    > data/genomes/bacterial/taxonomy/contig_to_taxid.tsv
...
' data/genomes/bacterial/taxonomy/contig_taxonomy.tsv \
  ...
  > data/genomes/bacterial/taxonomy/hq_chromosomes_labeled.fasta
